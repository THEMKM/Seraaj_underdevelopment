...
class PaymentError(SeraajError):
    ...
    def __init__(self, message: str = "Payment processing failed", payment_id: str = None, details: Dict[str, Any] = None):
...
class ErrorHandler:
    ...
    async def _handle_seraaj_error(self, error: SeraajError, error_id: str) -> JSONResponse:
    ...
    async def _handle_validation_error(self, error: RequestValidationError, error_id: str) -> JSONResponse:
    ...
    async def _handle_http_exception(self, error: HTTPException, error_id: str) -> JSONResponse:
        ...
        response_data = {
            ...
            "message": error.detail if error.detail else self.error_mappings.get(error.status_code, "HTTP Error"),
            "timestamp": datetime.now(datetime.timezone.utc).isoformat(),
            "details": {}
        }
    ...
    async def _handle_starlette_exception(self, error: StarletteHTTPException, error_id: str) -> JSONResponse:
        ...
        response_data = {
            ...
            "error_id": error_id,
            "error_code": f"HTTP_{error.status_code}",
            "message": error.detail if hasattr(error, 'detail') else self.error_mappings.get(error.status_code, "HTTP Error"),
            "timestamp": datetime.now(datetime.timezone.utc).isoformat(),
            "details": {}
        }
    ...
    async def _handle_database_error(self, error: SQLAlchemyError, error_id: str) -> JSONResponse:
    ...
    async def _handle_pydantic_error(self, error: ValidationError, error_id: str) -> JSONResponse:
    ...
    async def _handle_generic_error(self, error: Exception, error_id: str) -> JSONResponse:
...
def raise_not_found(message: str = "Resource not found", resource_type: str = None, resource_id: str = None):
...
def raise_external_service_error(message: str, service: str, details: Dict[str, Any] = None):
