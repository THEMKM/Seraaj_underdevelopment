...
class TestAPIPerformance:
    ...
    def test_opportunity_list_response_time(self, client: TestClient, session: Session, test_user_organization: User, performance_timer):
        """Test opportunities list endpoint response time"""
        # Create test data
        TestDataFactory.create_multiple_opportunities(session, test_user_organization, count=100)
    ...
    def test_opportunity_search_performance(self, client: TestClient, session: Session, test_user_organization: User, performance_timer):
        """Test search endpoint performance with large dataset"""
        # Create diverse opportunities
        TestDataFactory.create_multiple_opportunities(session, test_user_organization, count=200)
    ...
    def test_user_authentication_performance(self, client: TestClient, performance_timer):
    ...
    def test_opportunity_creation_performance(self, client: TestClient, auth_headers_organization: dict, performance_timer):
...
class TestConcurrentUsers:
    ...
    def test_concurrent_opportunity_browsing(self, client: TestClient, session: Session, test_user_organization: User):
        """Test multiple users browsing opportunities simultaneously"""
        # Create test data
        TestDataFactory.create_multiple_opportunities(session, test_user_organization, count=50)
    ...
    def test_concurrent_application_submissions(
        ...
        def submit_application(volunteer_index):
            ...
            start_time = time.time()
            response = client.post("/v1/applications", json=application_data, headers=headers)
            end_time = time.time()
            ...
            return {
                ...
                "status_code": response.status_code,
                "response_time": end_time - start_time,
                "success": response.status_code in [201, 400]  # 400 might be duplicate application
            }
        ...
        if valid_results:
            avg_response_time = sum(result["response_time"] for result in valid_results) / len(valid_results)
            assert avg_response_time < 3.0
...
class TestDatabasePerformance:
    ...
    def test_large_dataset_query_performance(self, client: TestClient, session: Session, test_user_organization: User, performance_timer):
        """Test query performance with large datasets"""
        # Create large dataset
        TestDataFactory.create_multiple_opportunities(session, test_user_organization, count=500)
    ...
    def test_pagination_performance(self, client: TestClient, session: Session, test_user_organization: User, performance_timer):
        """Test pagination performance with large datasets"""
        # Create large dataset
        TestDataFactory.create_multiple_opportunities(session, test_user_organization, count=1000)
        ...
        for case in test_cases:
            performance_timer.start()
            response = client.get(f"/v1/opportunities?skip={case['skip']}&limit={case['limit']}")
            performance_timer.stop()
...
class TestMemoryAndResourceUsage:
    ...
    def test_memory_usage_with_large_responses(self, client: TestClient, session: Session, test_user_organization: User):
    ...
    def test_file_upload_performance(self, client: TestClient, auth_headers_volunteer: dict, performance_timer):
...
class TestCachePerformance:
    ...
    def test_repeated_requests_performance(self, client: TestClient, session: Session, test_user_organization: User):
    ...
    def test_search_index_performance(self, client: TestClient, session: Session, test_user_organization: User, performance_timer):
        """Test search performance assumes proper indexing"""
        # Create large searchable dataset
        TestDataFactory.create_multiple_opportunities(session, test_user_organization, count=300)
...
class TestLoadTesting:
    ...
    def test_sustained_load_simulation(self, client: TestClient, session: Session, test_user_organization: User):
        ...
        print(f"Load test results: {len(results)} requests, {success_rate:.2%} success rate, "
