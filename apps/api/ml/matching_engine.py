...
from models import (
    ...
    Review, UserActivity, AnalyticsEvent
)
...
class MatchingEngine:
        ...
        for opportunity in opportunities:
            ...
            if score > 0.3:  # Minimum threshold
                matches.append({
                    ...
                    'organization_name': await self._get_organization_name(session, opportunity.org_id),
    ...
    def _calculate_availability_match(
        ...
        key = (
            ...
            opportunity_time_commitment.value if opportunity_time_commitment else 'flexible'
        )
    ...
    def _calculate_cause_match(
        self, 
        volunteer_causes: List[str], 
        opportunity_causes: List[str]
    ) -> float:
    ...
    def _calculate_time_commitment_match(
        ...
        ratio = min(volunteer_time, opportunity_time) / max(volunteer_time, opportunity_time)
    ...
    async def _calculate_personalization_boost(
        ...
        successful_apps = [app for app in applications if app.status in ['accepted', 'completed']]
    ...
    async def _get_match_reasons(
        ...
        # Skill matches
        matching_skills = set(volunteer.skills).intersection(set(opportunity.skills_required))
        if matching_skills:
        ...
        if volunteer.experience_level and opportunity.experience_required:
            if volunteer.experience_level.value == opportunity.experience_required.value:
                reasons.append("Perfect experience match")
            elif volunteer.experience_level.value > opportunity.experience_required.value:
                reasons.append("Experienced candidate")
        
        # Cause alignment
        matching_causes = set(volunteer.preferred_causes).intersection(set(opportunity.causes))
        if matching_causes:
    ...
    async def _get_volunteer_match_reasons(
        ...
        return await self._get_match_reasons(session, volunteer, opportunity, total_score)
        ...
        try:
            ...
            match_event = session.exec(
                select(AnalyticsEvent).where(
                    and_(
                        ...
                        AnalyticsEvent.data.op("->>")(["volunteer_id"]) == str(volunteer_id),
                        AnalyticsEvent.data.op("->>")(["opportunity_id"]) == str(opportunity_id)
                    )
                )
            ).first()
            ...
            for feature, score in feature_scores.items():
                if feature in self.feature_weights:
                    ...
                    self.feature_weights[feature] = max(0.01, self.feature_weights[feature])
