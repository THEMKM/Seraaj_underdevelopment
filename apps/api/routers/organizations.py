...
from models import (
    ...
    OpportunityRead, ApplicationRead
)
...
async def get_organization_applications(
    ...
    opportunity_id: Optional[int] = Query(None, description="Filter by specific opportunity")
):
    ...
    query = select(Application).join(Opportunity).where(
...
async def get_organization_dashboard_stats(
    ...
    total_opportunities = session.exec(
        select(func.count(Opportunity.id)).where(Opportunity.org_id == organization.id)
    ).first() or 0
    
    active_opportunities = session.exec(
        select(func.count(Opportunity.id)).where(
            and_(Opportunity.org_id == organization.id, Opportunity.state == "active")
        )
    ).first() or 0
    
    total_applications = session.exec(
        select(func.count(Application.id)).join(Opportunity).where(
            Opportunity.org_id == organization.id
        )
    ).first() or 0
...
async def get_organization_reviews(
    ...
    query = select(Review).where(
        Review.reviewed_organization_id == organization.id
