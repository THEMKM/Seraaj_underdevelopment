...
from models import (
    ...
    Message, Conversation, ConversationParticipant, AnalyticsEvent
)
...
class TeamCollaborationManager:
    ...
    def create_team_workspace(
        ...
        logger.info(f"Team workspace created: {workspace_name} for organization {organization_id}")
        return workspace
    ...
    def _add_team_member(
        ...
        member = {
            ...
            "permissions": [p.value for p in self.role_permissions.get(TeamRole(role), [])],
            "active": True,
    ...
    def _create_milestone(self, milestone_data: Dict[str, Any], project_id: str) -> Dict[str, Any]:
    ...
    def assign_mentor(
        ...
        self._log_collaboration_event(
            session, "mentor_assigned", mentorship_id, assigned_by or mentor_id, mentorship
        )
    ...
    def generate_team_report(
        ...
        self._log_collaboration_event(
            session, "report_generated", report_id, generated_by, 
    ...
    def schedule_team_meeting(
        ...
        meeting = {
            ...
            "type": meeting_data.get("type", "general"),  # standup, planning, review, etc.
            ...
            "recent_activity": [
                {
                    ...
                    "timestamp": (datetime.now(datetime.timezone.utc) - timedelta(hours=2)).isoformat()
                },
                {
                    "type": "new_volunteer",
                    "user": "System",
                    "description": "3 new volunteers joined the Spring Cleanup project",
                    "timestamp": (datetime.now(datetime.timezone.utc) - timedelta(hours=4)).isoformat()
                }
            ],
            "upcoming_deadlines": [
                {
                    ...
                    "due_date": (datetime.now(datetime.timezone.utc) + timedelta(days=3)).isoformat(),
    ...
    def _calculate_engagement_metrics(self, events: List[AnalyticsEvent]) -> Dict[str, Any]:
        """Calculate team engagement metrics"""
        
        return {
            "active_users": len(set(e.user_id for e in events if e.user_id)),
            "total_actions": len(events),
            "average_actions_per_user": round(len(events) / max(len(set(e.user_id for e in events if e.user_id)), 1), 2),
    ...
    def _calculate_productivity_metrics(self, events: List[AnalyticsEvent]) -> Dict[str, Any]:
    ...
    def _calculate_communication_metrics(self, events: List[AnalyticsEvent]) -> Dict[str, Any]:
