...
class CalendarEvent:
    ...
    end_datetime: datetime = datetime.now(datetime.timezone.utc) + timedelta(hours=1)
    location: str = ""
...
class CalendarScheduler:
    ...
    def create_event(
        ...
        if not event_data.id:
            event_data.id = hash(f"{event_data.title}_{event_data.start_datetime}_{uuid.uuid4()}")
        ...
        logger.info(f"Calendar event created: {event_data.title} at {event_data.start_datetime}")
        return event_data
    ...
    def _validate_event_data(self, event_data: CalendarEvent):
        ...
        if event_data.start_datetime < datetime.now(datetime.timezone.utc) - timedelta(minutes=5):
        ...
        if not event_data.participant_ids and event_data.event_type != EventType.DEADLINE_REMINDER:
    ...
    def _is_within_business_hours(self, start_time: datetime, end_time: datetime) -> bool:
        ...
        if (start_time_only < self.business_hours["start"] or 
            end_time_only > self.business_hours["end"]):
            return False
    ...
    def schedule_interview(
        ...
        best_time = self._find_best_time_slot(
            session, proposed_times, duration_minutes, 
    ...
    def schedule_recurring_volunteer_sessions(
        ...
        start_date: date,
        end_date: date,
        session_times: List[Dict[str, Any]],  # [{"day": 1, "start_time": "09:00", "duration": 240}]
        volunteer_ids: List[int],
        location: str = "",
        virtual_meeting_url: str = ""
    ) -> List[CalendarEvent]:
            ...
            for session_time in session_times:
                ...
                if current_date.weekday() == session_day:
                    ...
                    try:
                        ...
                        logger.warning(f"Could not schedule session on {current_date}: {e}")
    ...
    def get_user_calendar(
        ...
        if event_types:
            query = query.where(CalendarEvent.event_type.in_([et.value for et in event_types]))
    ...
    def get_availability_slots(
        ...
        while current_date <= end_date:
            ...
            if current_date.weekday() < 5:
                ...
                current_time = datetime.combine(current_date, self.business_hours["start"])
                ...
                while current_time + timedelta(minutes=duration_minutes) <= end_of_day:
                    ...
                    blocking_conflicts = [c for c in conflicts if c["type"] != "business_hours"]
                    ...
                    if not blocking_conflicts:
                        available_slots.append({
                            ...
                            "quality_score": self._calculate_slot_quality(current_time)
                        })
    ...
    def _schedule_reminders(self, session: Session, event_data: CalendarEvent):
        ...
        for minutes_before in event_data.reminder_minutes:
            reminder_time = event_data.start_datetime - timedelta(minutes=minutes_before)
            
            # In a real implementation, this would create reminder tasks
            # For now, just log the reminder scheduling
            logger.info(f"Reminder scheduled for {reminder_time} ({minutes_before} min before {event_data.title})")
    ...
    def _notify_participants(
        ...
        for participant_id in event_data.participant_ids:
            ...
            logger.info(f"Notification '{notification_type}' sent to user {participant_id} for event: {event_data.title}")
    ...
    def reschedule_event(
        ...
        conflicts = self._check_scheduling_conflicts(
            session, new_start_time, new_end_time, []  # Would get participant IDs from existing event
        )
        
        if conflicts:
            serious_conflicts = [c for c in conflicts if c["type"] != "business_hours"]
            if serious_conflicts:
                raise ValueError(f"Cannot reschedule due to conflicts: {serious_conflicts}")
        ...
        # Log the reschedule
        self._log_calendar_event(
            session, "event_rescheduled", event_id, rescheduled_by or 0,
    ...
    def cancel_event(
        ...
        self._log_calendar_event(
            session, "event_cancelled", event_id, cancelled_by or 0,
