...
class GuidedTourService:
    ...
    async def get_available_tours(
        ...
        try:
            ...
            completed_tour_ids = [
                p.tour_id for p in user_progress if p.status == TourStatus.COMPLETED
            ]
            ...
            for tour in tours:
                ...
                progress = next(
                    (p for p in user_progress if p.tour_id == tour.id), None
                )
                ...
                tour_data = {
                    ...
                    "progress": {
                        "status": (
                            progress.status if progress else TourStatus.NOT_STARTED
                        ),
    ...
    async def get_tour_step(
        self, user_id: int, tour_id: str, step_number: int
    ) -> Dict[str, Any]:
    ...
    async def advance_tour_step(
        self, user_id: int, tour_id: str, step_number: int, action: str = "next"
    ) -> Dict[str, Any]:
    ...
    async def submit_tour_feedback(
        ...
        try:
            ...
            logger.info(
                f"Tour feedback submitted: user {user_id}, tour {tour_id}, rating {rating}"
            )
            return True
    ...
    async def get_tour_analytics(self, tour_id: str, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            skipped_users = len(
                [p for p in all_progress if p.status == TourStatus.SKIPPED]
            )
            in_progress_users = len(
                [p for p in all_progress if p.status == TourStatus.IN_PROGRESS]
            )
            ...
            completed_progress = [
                p for p in all_progress if p.completed_at and p.started_at
            ]
    ...
    async def create_tour_from_template(
        ...
        try:
            ...
            tour = GuidedTour(
                ...
                tour_type=TourType(
                    customizations.get("tour_type", template_data["tour_type"])
                ),
                target_role=TourUserRole(
                    customizations.get("target_role", template.target_role)
                ),
                entry_url=customizations.get(
                    "entry_url", template_data.get("entry_url", "/")
                ),
                estimated_duration_minutes=customizations.get(
                    "duration", template_data.get("duration", 5)
                ),
    ...
    async def _check_tour_eligibility(
        self, tour: GuidedTour, user_id: int, current_url: Optional[str]
    ) -> bool:
    ...
    async def _get_tour_step(
        self, tour_id: int, step_number: int
    ) -> Optional[Dict[str, Any]]:
        """Get a specific tour step"""

        step = self.session.exec(
            select(TourStep).where(
                and_(TourStep.tour_id == tour_id, TourStep.step_number == step_number)
            )
        ).first()
    ...
    async def _get_tour_step_count(self, tour_id: int) -> int:
        ...
        return len(
            self.session.exec(select(TourStep).where(TourStep.tour_id == tour_id)).all()
        )

    def _calculate_completion_percentage(
        self, tour: GuidedTour, progress: Optional[UserTourProgress]
    ) -> float:
        ...
        total_steps = len(
            self.session.exec(select(TourStep).where(TourStep.tour_id == tour.id)).all()
        )
    ...
    async def _track_tour_event(self, tour_id: int, event_type: str, user_id: int):
        ...
        try:
            ...
            logger.info(
                f"Tour event: {event_type} for tour {tour_id} by user {user_id}"
            )
        except Exception as e:
    ...
    async def _get_feedback_summary(
        self, tour_id: int, since_date: datetime
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "positive_feedback": len(
                [f for f in feedback_records if f.rating and f.rating >= 4]
            ),
            "negative_feedback": len(
                [f for f in feedback_records if f.rating and f.rating <= 2]
            ),
