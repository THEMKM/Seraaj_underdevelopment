...
from models import (
    User, UserRead, UserUpdate, UserRole, UserStatus,
    Volunteer, Organisation, Opportunity, Application, 
    Message, Conversation, Review, AnalyticsEvent,
    DailyStats, UserActivity, PerformanceMetric
)
...
async def get_opportunity_statistics(
    ...
    state_breakdown = session.exec(
        select(Opportunity.state, func.count(Opportunity.id))
    ...
    return {
        ...
        "most_active_organizations": [
            {"name": name, "opportunity_count": count}
            for name, count in popular_orgs
        ]
    }
    ...
    recent_errors = session.exec(
        ...
        .where(
            and_(
                ...
                AnalyticsEvent.timestamp >= datetime.now(datetime.timezone.utc) - timedelta(hours=1)
            )
        )
    ).first()
...
async def clear_sample_data(
    ...
    try:
        ...
        tables_to_clear = [
            "skill_verifications", "reviews", "calendar_events", 
            "applications", "opportunities", "organisations", 
