...
class DemoScenario(SQLModel, table=True):
    ...
    scenario_id: str = Field(default_factory=lambda: str(uuid.uuid4()), unique=True, index=True)
...
class DemoRun(SQLModel, table=True):
    ...
    id: Optional[int] = Field(default=None, primary_key=True)
    run_id: str = Field(default_factory=lambda: str(uuid.uuid4()), unique=True, index=True)
    ...
    error_logs: List[Dict[str, Any]] = Field(default_factory=list, sa_column=Column(JSON))
...
class DemoTemplate(SQLModel, table=True):
    ...
    default_settings: Dict[str, Any] = Field(default_factory=dict, sa_column=Column(JSON))
...
class DemoAsset(SQLModel, table=True):
    ...
    id: Optional[int] = Field(default=None, primary_key=True)
    asset_id: str = Field(default_factory=lambda: str(uuid.uuid4()), unique=True, index=True)
...
class DemoAnalytics(SQLModel, table=True):
    ...
    # Technical metrics
    browser_breakdown: Dict[str, int] = Field(default_factory=dict, sa_column=Column(JSON))
    device_breakdown: Dict[str, int] = Field(default_factory=dict, sa_column=Column(JSON))
...
class DemoFeedback(SQLModel, table=True):
    ...
    # Feedback details
    feedback_type: str = Field(default="general")  # general, bug_report, suggestion, improvement
    rating: Optional[int] = Field(ge=1, le=5)
    title: str
    message: str
