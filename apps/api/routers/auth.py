...
from models import User, UserCreate, UserLogin, UserRead, Token, TokenRefresh, UserUpdate
...
from middleware.error_handler import (
    ...
    AuthenticationError, BusinessLogicError
)
...
async def login(
    ...
    if hasattr(user, 'status'):
        ...
        if (hasattr(user, 'status') and user.status != "active") or (hasattr(user, 'is_active') and not user.is_active):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Account is not active"
            )
...
async def refresh_token(
    ...
    if not user or (hasattr(user, 'status') and user.status != "active") or (hasattr(user, 'is_active') and not user.is_active):
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
