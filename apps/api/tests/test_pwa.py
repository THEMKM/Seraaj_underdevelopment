...
class TestPWAManifest:
    ...
    def test_authenticated_manifest_generation(self, client: TestClient, auth_headers_volunteer: dict):
    ...
    def test_role_specific_manifest(self, client: TestClient):
        ...
        assert response.status_code == 200
        manifest = response.json()
        assert "volunteer" in manifest["name"].lower() or "opportunities" in str(manifest.get("shortcuts", []))
...
class TestInstallPrompt:
    ...
    def test_install_prompt_authenticated(self, client: TestClient, auth_headers_volunteer: dict):
...
class TestOfflineData:
    ...
    def test_sync_status_update(self, client: TestClient, auth_headers_volunteer: dict):
        ...
        response = client.post("/pwa/sync-status", json=sync_data, headers=auth_headers_volunteer)
    ...
    def test_cache_info(self, client: TestClient, auth_headers_volunteer: dict):
        ...
        assert strategies["opportunities"] in ["network_first", "cache_first", "stale_while_revalidate"]
...
class TestPWACapabilities:
    ...
    def test_authenticated_capabilities(self, client: TestClient, auth_headers_organization: dict):
...
class TestShareTarget:
    ...
    def test_share_with_authentication(self, client: TestClient, auth_headers_volunteer: dict):
        ...
        response = client.post("/pwa/share", data=share_data, headers=auth_headers_volunteer)
...
class TestPWAAdmin:
    ...
    def test_generate_files_unauthorized(self, client: TestClient, auth_headers_volunteer: dict):
    ...
    def test_storage_schema_debug(self, client: TestClient, auth_headers_volunteer: dict):
        """Test storage schema debug endpoint"""
        response = client.get("/pwa/debug/storage-schema", headers=auth_headers_volunteer)
...
class TestPWAPerformance:
    ...
    def test_manifest_compression(self, client: TestClient):
        ...
        assert "  " not in manifest_str or manifest_str.count("  ") < 10  # Minimal whitespace
