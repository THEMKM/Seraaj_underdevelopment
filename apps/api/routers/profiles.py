...
async def create_volunteer_profile(
    ...
    db_volunteer = Volunteer(**volunteer_data.model_dump(), user_id=current_user.id)
...
async def create_organization_profile(
    ...
    # Create organization profile
    db_org = Organisation(**org_data.model_dump(), user_id=current_user.id)
...
async def get_volunteer_stats(
    volunteer_id: int, session: Annotated[Session, Depends(get_session)]
):
    ...
    return {
        ...
        "completion_rate": (
            (completed_applications / accepted_applications * 100)
...
async def get_organization_stats(
    ...
    active_opportunities = session.exec(
        select(func.count(Opportunity.id)).where(
            and_(Opportunity.org_id == org_id, Opportunity.state == "active")
        )
    ).first()
