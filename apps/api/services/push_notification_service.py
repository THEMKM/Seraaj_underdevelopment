...
class PushNotificationService:
    ...
    def __init__(self, session: Session):
        ...
        self.vapid_claims = {"sub": "mailto:notifications@seraaj.org"}
    ...
    def _get_vapid_private_key(self) -> str:
        ...
        return getattr(
            settings, "vapid_private_key", "demo-vapid-private-key-placeholder"
        )

    def _get_vapid_public_key(self) -> str:
        """Get VAPID public key from settings"""
        return getattr(
            settings, "vapid_public_key", "demo-vapid-public-key-placeholder"
        )
    ...
    async def subscribe_user(
        ...
        try:
            ...
            if existing_subscription:
                ...
                if device_info:
                    existing_subscription.device_type = device_info.get(
                        "device_type", "unknown"
                    )
    ...
    async def send_notification(
        ...
        try:
            ...
            logger.info(
                f"Created {len(notifications)} notifications for user {user_id}"
            )
    ...
    async def _deliver_notification(self, notification: PushNotification) -> bool:
        ...
        try:
            subscription = self.session.get(
                PushSubscription, notification.subscription_id
            )
            ...
            if await self._is_quiet_hours(subscription):
                ...
                notification.scheduled_at = await self._calculate_next_send_time(
                    subscription
                )
                self.session.add(notification)
                self.session.commit()
            ...
            await self._log_delivery_attempt(
                notification, subscription.platform, success
            )
    ...
    async def _send_web_push(
        ...
        try:
            ...
            if PYWEBPUSH_AVAILABLE:
                ...
                logger.info(
                    f"[DEV MODE] pywebpush not available, simulating web push: {payload}"
                )
                return True
    ...
    def _prepare_notification_payload(
        ...
        payload["options"] = {
            "requireInteraction": notification.priority
    ...
    async def _log_delivery_attempt(
        self, notification: PushNotification, platform: PlatformType, success: bool
    ):
            ...
            failed_notifications = self.session.exec(
                select(PushNotification).where(
                    and_(
                        ...
                        > datetime.now(datetime.timezone.utc) - timedelta(hours=24),
                    )
                )
            ).all()
