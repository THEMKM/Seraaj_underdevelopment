...
class DatabaseOptimizer:
            ...
            for index_def in indexes_to_create:
                try:
                    ...
                    if index_def.get("optional", False):
                        ...
                        if not result:
                            logger.info(
                                f"Skipping index {index_def['name']} - table {index_def['table']} does not exist"
                            )
                            continue

                        # Also check if all required columns exist
                        table_info = conn.execute(
                            text(f"PRAGMA table_info({index_def['table']})")
                        ).fetchall()
                        ...
                        if missing_columns:
                            logger.info(
                                f"Skipping index {index_def['name']} - missing columns: {missing_columns}"
                            )
                            continue
                    ...
                    if result:
                        logger.info(
                            f"Index {index_def['name']} already exists, skipping"
                        )
                        continue
        ...
        test_queries = [
            {
                ...
                .where(Opportunity.state == "active")
    ...
    def get_database_stats(self, session: Session) -> Dict[str, Any]:
        ...
        stats = {"tables": {}, "indexes": {}, "performance": {}}
...
def optimize_database() -> Dict[str, Any]:
    ...
    results = {"indexes": {}, "vacuum": {}, "analyze": {}, "performance": {}}
...
def get_database_health() -> Dict[str, Any]:
    ...
    with Session(optimizer.engine) as session:
        ...
        error_tables = [
            name for name, data in stats["tables"].items() if "error" in data
        ]
