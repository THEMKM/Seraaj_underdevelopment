...
class TestWebSocketConnection:
    ...
    def test_websocket_connection_authenticated(
        ...
        with client.websocket_connect(f"/v1/ws/{test_user_volunteer.id}?token={token}") as websocket:
...
class TestWebSocketMessaging:
    ...
    def test_send_direct_message(
        ...
        with client.websocket_connect(f"/v1/ws/{test_user_volunteer.id}?token={volunteer_token}") as websocket:
...
class TestWebSocketNotifications:
    ...
    def test_new_message_notification(
        ...
        with client.websocket_connect(f"/v1/ws/{test_user_organization.id}?token={token}") as websocket:
            try:
                response = websocket.receive_json(timeout=5)
...
class TestWebSocketPerformance:
    ...
    def test_message_rate_limiting(
        self, 
        client: TestClient, 
        test_user_volunteer: User
    ):
    ...
    def test_concurrent_connections(
        ...
        try:
            with client.websocket_connect(f"/v1/ws/{test_user_volunteer.id}?token={volunteer_token}") as ws1:
                with client.websocket_connect(f"/v1/ws/{test_user_organization.id}?token={org_token}") as ws2:
