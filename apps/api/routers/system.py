...
from middleware.error_handler import raise_forbidden
from middleware.loading_states import ProgressTracker, loading_manager, async_operation
...
async def detailed_health_check(
    ...
    loading_operations = {
        "active_operations": len(
            [
                state
                for state in loading_manager.loading_states.values()
    ...
    return success_with_data(
        health_data, "Detailed system health retrieved successfully"
    )
    ...
    status_data = {
        ...
        "performance": {
            "avg_response_time_ms": round(
                sum(
                    response_time
                    for response_time in loading_manager.performance_history.get(
                        "response_times", []
                    )
                )
                / max(
                    len(loading_manager.performance_history.get("response_times", [])),
                    1,
                ),
                2,
            ),
...
async def toggle_maintenance_mode(
    ...
    settings.maintenance_message = message or (
        "System is under maintenance" if enable else "System is operational"
    )

    maintenance_status = {
        "maintenance_mode": enable,
        "message": message
        or ("System is under maintenance" if enable else "System is operational"),
    ...
    logger.info(
        f"Maintenance mode {'enabled' if enable else 'disabled'} by user {current_user.id}"
    )
...
async def perform_cache_clear(
    cache_type: str, user_id: int, progress_tracker: ProgressTracker = None
):
    ...
    return {
        ...
        "cleared_at": datetime.now(datetime.timezone.utc).isoformat(),
    }
...
async def system_metrics(
    ...
    metrics = {
        ...
        "api_usage": {
            "total_requests": len(
                loading_manager.performance_history.get("request_counts", [])
            ),
            "error_rate": (
                len(
                    [
                        c
                        for c in loading_manager.performance_history.get(
                            "error_counts", []
                        )
                        if c > 0
                    ]
                ...
                / max(
                    len(loading_manager.performance_history.get("error_counts", [])), 1
                )
            )
            * 100,
            "avg_response_time_ms": round(
        ...
        "system_performance": {
            "database_connections": getattr(
                session.get_bind().pool, "checkedout", lambda: 0
            )(),
            "active_operations": len(
                [
                    state
    ...
    return success_with_data(
        metrics, f"System metrics for {time_range} retrieved successfully"
    )
...
async def perform_backup(
    backup_type: str, user_id: int, progress_tracker: ProgressTracker = None
):
