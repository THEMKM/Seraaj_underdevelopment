...
from utils.response_formatter import created_resource, format_api_response
...
async def create_opportunity(
    ...
    try:
        ...
        raise_bad_request(
            "Opportunity data validation failed", details={"validation_error": str(e)}
        )
    ...
    db_opportunity = Opportunity(**validated_data, org_id=organization.id)
    ...
    return created_resource(
        opportunity_read.model_dump(), "opportunity", str(db_opportunity.id)
    )
...
async def get_opportunities(
    ...
    if causes:
        ...
        for cause in causes:
            query = query.where(
                func.json_extract(Opportunity.causes, "$").contains(cause)
            )
    ...
    if skills:
        ...
        for skill in skills:
            query = query.where(
                func.json_extract(Opportunity.skills_required, "$").contains(skill)
            )
...
@router.get(
    "/{opportunity_id}/applications", response_model=OpportunityApplicationsResponse
)
...
async def get_featured_opportunities(
    ...
    query = (
        select(Opportunity)
        .where(and_(Opportunity.featured == True, Opportunity.state == "active"))
...
async def get_opportunities_stats(session: Annotated[Session, Depends(get_session)]):
