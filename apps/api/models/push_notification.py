...
class PushSubscription(SQLModel, table=True):
    ...
    # User relationship - temporarily disabled until User model relationships are restored
    user_id: int = Field(foreign_key="users.id", index=True)
    # user: "User" = Relationship(back_populates="push_subscriptions")  # Disabled due to missing relationship on User model
    ...
    notifications: List["PushNotification"] = Relationship(
...
class PushNotification(SQLModel, table=True):
    ...
    # User and subscription - temporarily disabled until User model relationships are restored
    user_id: int = Field(foreign_key="users.id", index=True)
    # user: "User" = Relationship(back_populates="push_notifications")  # Disabled due to missing relationship on User model

    subscription_id: Optional[int] = Field(foreign_key="push_subscriptions.id")
    subscription: Optional["PushSubscription"] = Relationship(
...
class NotificationTemplate(SQLModel, table=True):
    ...
    default_actions: Optional[List[Dict[str, Any]]] = Field(
        default=None, sa_column=Column(JSON)
    )
...
class NotificationSettings(SQLModel, table=True):
    ...
    # user: "User" = Relationship(back_populates="notification_settings")  # Disabled due to missing relationship on User model
