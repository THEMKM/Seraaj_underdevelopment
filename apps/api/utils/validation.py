...
from middleware.error_handler import DataValidationError
...
def validate_email(email: str) -> str:
    ...
    if not re.match(email_pattern, email):
        raise DataValidationError("Invalid email format", field="email", value=email)
    return email.lower()
...
def validate_password_strength(password: str) -> str:
    ...
    if not re.search(r"\d", password):
        raise DataValidationError(
            "Password must contain at least one number", field="password"
        )
...
def validate_phone_number(phone: str) -> str:
    ...
    if not re.match(phone_pattern, cleaned_phone):
        raise DataValidationError(
            "Invalid phone number format", field="phone_number", value=phone
        )
...
def validate_date_range(
    start_date: date, end_date: date, field_prefix: str = ""
) -> tuple:
    """Validate that end date is after start date"""
    if start_date >= end_date:
        raise DataValidationError(
            "End date must be after start date", field=f"{field_prefix}date_range"
        )
...
def validate_skills_list(skills: List[str]) -> List[str]:
    ...
    if len(skills) > 20:
        raise DataValidationError(
            "Maximum 20 skills allowed", field="skills", value=len(skills)
        )
    ...
    for skill in skills:
        if not skill or len(skill.strip()) < 2:
            raise DataValidationError(
                "Each skill must be at least 2 characters long", field="skills"
            )

        if len(skill) > 50:
            raise DataValidationError(
                "Each skill must be 50 characters or less", field="skills", value=skill
            )
...
def validate_url(url: str, field_name: str = "url") -> str:
    ...
    if not re.match(url_pattern, url):
        raise DataValidationError("Invalid URL format", field=field_name, value=url)
...
def validate_numeric_range(
    value: float, field_name: str, min_value: float = None, max_value: float = None
) -> float:
    """Validate numeric value within range"""
    if min_value is not None and value < min_value:
        raise DataValidationError(
            f"{field_name} must be at least {min_value}", field=field_name, value=value
        )

    if max_value is not None and value > max_value:
        raise DataValidationError(
            f"{field_name} must be at most {max_value}", field=field_name, value=value
        )
...
def validate_opportunity_data(opportunity_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "volunteers_needed" in opportunity_data:
        validated_data["volunteers_needed"] = int(
