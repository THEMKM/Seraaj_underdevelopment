...
from models.guided_tour import (
    ...
    TourType, TourStatus, TourUserRole
)
...
async def get_available_tours(
    ...
    current_url: Optional[str] = Query(None, description="Current page URL for context")
):
...
async def submit_tour_feedback(
    ...
    try:
        ...
        if success:
            return success_with_data({"submitted": True}, "Feedback submitted successfully")
        else:
            return JSONResponse("Tour not found", 404)
...
async def get_tour_progress(
    ...
    try:
        ...
        total_steps = len(session.exec(
            select(TourStep).where(TourStep.tour_id == tour.id)
        ).all())
        
        completion_percentage = (len(progress.completed_steps) / total_steps * 100) if total_steps > 0 else 0
...
async def get_tour_templates(
    ...
    target_role: Optional[TourUserRole] = Query(None, description="Filter by target role"),
    ...
    try:
        ...
        templates = session.exec(query.order_by(TourTemplate.is_featured.desc(), TourTemplate.usage_count.desc())).all()
...
async def get_tours_overview(
    ...
    try:
        ...
        # Get tour statistics
        tours = session.exec(select(GuidedTour).where(GuidedTour.is_active == True)).all()
        ...
        total_completions = len([p for p in all_progress if p.status == TourStatus.COMPLETED])
        ...
        for tour in tours:
            ...
            completions = len([p for p in tour_progress if p.status == TourStatus.COMPLETED])
...
async def get_my_tour_progress(
    ...
    try:
        ...
        return success_with_data({
            ...
            "completed_tours": len([p for p in progress_data if p["status"] == TourStatus.COMPLETED]),
            "in_progress_tours": len([p for p in progress_data if p["status"] == TourStatus.IN_PROGRESS])
