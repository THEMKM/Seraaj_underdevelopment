...
async def get_vapid_public_key():
    ...
    try:
        ...
        if settings.environment.value == "development":
            ...
            return success_with_data({"public_key": demo_key}, "VAPID public key retrieved")
        ...
        return success_with_data({"public_key": public_key}, "VAPID public key retrieved")
...
async def unsubscribe_from_notifications(
    ...
    try:
        ...
        if success:
            return success_with_data({"unsubscribed": True}, "Successfully unsubscribed from notifications")
        else:
...
async def get_notification_history(
    ...
    try:
        ...
        query = query.order_by(PushNotification.created_at.desc()).offset(offset).limit(limit)
...
async def track_notification_click(
    ...
    try:
        notification = session.exec(
            select(PushNotification).where(PushNotification.notification_id == notification_id)
        ).first()
        ...
        logger.info(f"Notification {notification_id} clicked by user {user.id if user else 'anonymous'}")
...
async def get_notification_statistics(
    ...
    try:
        ...
        stats = {
            "total_sent": len([n for n in total_notifications if n.status == NotificationStatus.SENT]),
            "total_delivered": len([n for n in total_notifications if n.status == NotificationStatus.DELIVERED]),
            "total_clicked": len([n for n in total_notifications if n.status == NotificationStatus.CLICKED]),
            "total_failed": len([n for n in total_notifications if n.status == NotificationStatus.FAILED]),
            "by_type": {},
            "by_priority": {},
        ...
        if stats["total_sent"] > 0:
            stats["click_through_rate"] = (stats["total_clicked"] / stats["total_sent"]) * 100
...
async def send_test_notification(
    ...
    try:
        ...
        notifications = await service.send_notification(
            ...
            data={"test": True, "timestamp": datetime.now(datetime.timezone.utc).isoformat()},
