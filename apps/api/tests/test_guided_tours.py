...
class TestGuidedTourAPI:
    ...
    def test_get_available_tours(
        self, client: TestClient, auth_headers_volunteer: dict
    ):
        """Test getting available tours for user"""
        response = client.get("/guided-tours/available", headers=auth_headers_volunteer)
    ...
    def test_get_available_tours_with_filters(
        self, client: TestClient, auth_headers_volunteer: dict
    ):
    ...
    def test_start_tour_not_found(
        self, client: TestClient, auth_headers_volunteer: dict
    ):
        """Test starting non-existent tour"""
        response = client.post(
            "/guided-tours/start/non-existent-tour", headers=auth_headers_volunteer
        )
    ...
    def test_get_tour_progress_not_started(
        self, client: TestClient, auth_headers_volunteer: dict
    ):
        """Test getting progress for tour not started"""
        response = client.get(
            "/guided-tours/test-tour/progress", headers=auth_headers_volunteer
        )
    ...
    def test_skip_tour(
        ...
        response = client.post(
            f"/guided-tours/skip/{tour.tour_id}", headers=auth_headers_volunteer
        )
    ...
    def test_get_tour_templates(self, client: TestClient, auth_headers_volunteer: dict):
        ...
        response = client.get("/guided-tours/templates", headers=auth_headers_volunteer)
    ...
    def test_get_tour_templates_with_filters(
        self, client: TestClient, auth_headers_volunteer: dict
    ):
    ...
    def test_create_tour_requires_admin(
        self, client: TestClient, auth_headers_volunteer: dict
    ):
        ...
        response = client.post(
            "/guided-tours/create", json=tour_data, headers=auth_headers_volunteer
        )
    ...
    def test_get_tour_analytics_requires_admin(
        self, client: TestClient, auth_headers_volunteer: dict
    ):
        """Test that tour analytics requires admin access"""
        response = client.get(
            "/guided-tours/test-tour/analytics", headers=auth_headers_volunteer
        )
    ...
    def test_get_tours_overview_requires_admin(
        self, client: TestClient, auth_headers_volunteer: dict
    ):
        """Test that tours overview requires admin access"""
        response = client.get(
            "/guided-tours/analytics/overview", headers=auth_headers_volunteer
        )
    ...
    def test_get_my_tour_progress(
        self, client: TestClient, auth_headers_volunteer: dict
    ):
        """Test getting user's tour progress"""
        response = client.get(
            "/guided-tours/my-progress", headers=auth_headers_volunteer
        )
    ...
    def test_reset_tour_progress(
        ...
        response = client.post(
            f"/guided-tours/reset/{tour.tour_id}", headers=auth_headers_volunteer
        )
...
class TestGuidedTourService:
    ...
    def test_get_available_tours_empty(
        ...
        tours = asyncio.run(
            service.get_available_tours(
                user_id=test_user_volunteer.id, user_role=TourUserRole.VOLUNTEER
            )
        )

        assert tours == []

    def test_start_tour_creates_progress(
        self, session: Session, test_user_volunteer: User
    ):
    ...
    def test_skip_tour_updates_status(
        self, session: Session, test_user_volunteer: User
    ):
    ...
    def test_submit_tour_feedback_creates_record(
        self, session: Session, test_user_volunteer: User
    ):
        ...
        feedback_record = session.exec(
            select(TourFeedback).where(TourFeedback.user_id == test_user_volunteer.id)
        ).first()
    ...
    def test_tour_eligibility_check(self, session: Session, test_user_volunteer: User):
        ...
        is_eligible = asyncio.run(
            service._check_tour_eligibility(
                tour=tour, user_id=test_user_volunteer.id, current_url="/dashboard"
            )
        )
...
class TestTourTemplates:
    ...
    def test_create_tour_from_template(
        self, session: Session, test_user_volunteer: User
    ):
        ...
        tour_id = asyncio.run(
            service.create_tour_from_template(
...
class TestTourAnalytics:
    ...
    def test_tour_analytics_with_data(
        self, session: Session, test_user_volunteer: User, test_user_organization: User
    ):
