...
class DataSeeder:
    ...
    async def seed_sample_data(self, force: bool = False) -> Dict[str, Any]:
        ...
        events = await self._create_sample_calendar_events(
            organizations + volunteers, opportunities
        )
        ...
        for vol_data in volunteers_data:
            ...
            volunteer_profile = Volunteer(
                ...
                skills=[
                    ...
                    "Graphic Design",
                    "Teaching",
                ][user.id % 4 : user.id % 4 + 2],
                experience_years=2 + (user.id % 8),
                availability_hours=10 + (user.id % 20),
                preferred_causes=["Education", "Environment", "Health", "Technology"][
                    user.id % 4 : user.id % 4 + 2
                ],
    ...
    async def _create_sample_opportunities(
        self, organizations: List[User]
    ) -> List[Opportunity]:
        """Create sample opportunities"""
        opportunities = []
        ...
        for i, org_user in enumerate(organizations):
            ...
            for opp_info in opp_data:
                opportunity = Opportunity(
                    **opp_info, organization_id=org_user.id, created_by=org_user.id
                )
    ...
    async def _create_sample_applications(
        self, volunteers: List[User], opportunities: List[Opportunity]
    ) -> List[Application]:
        """Create sample applications"""
        applications = []
            ...
            for j, volunteer in enumerate(volunteers):
                ...
                application = Application(
                    ...
                    status=(
                        "pending"
                        if j % 3 == 0
                        else "accepted" if j % 3 == 1 else "reviewed"
                    ),
                )
    ...
    async def _create_sample_calendar_events(
        self, users: List[User], opportunities: List[Opportunity]
    ) -> List[CalendarEvent]:
        """Create sample calendar events"""
        events = []

        # Create events for the next few weeks
        start_date = datetime.now(datetime.timezone.utc).replace(
        ...
        for i in range(10):  # Create 10 sample events
            ...
            opportunity = (
                opportunities[i % len(opportunities)] if opportunities else None
            )
            ...
            event = CalendarEvent(
                ...
                virtual_meeting_url=(
                    "https://meet.example.com/room123" if i % 2 == 0 else None
                ),
    ...
    async def _create_sample_reviews(
        self, volunteers: List[User], organizations: List[User]
    ) -> List[Review]:
        """Create sample reviews"""
        reviews = []
        ...
        for i, volunteer in enumerate(volunteers[:2]):  # First 2 volunteers
            for j, org in enumerate(organizations[:2]):  # First 2 organizations
                ...
                vol_review = Review(
                    ...
                    opportunity_id=1,
                    rating=4.2 + (i + j) * 0.15,
                    comment="Excellent organization with clear communication and meaningful impact.",
    ...
    async def _create_sample_verifications(
        self, volunteers: List[User], skills_data: List[dict]
        ...
        for volunteer in volunteers:
            ...
            volunteer_skill_indices = list(
                range(
                    volunteer.id % len(skills_data),
                    min(volunteer.id % len(skills_data) + 2, len(skills_data)),
                )
            )
