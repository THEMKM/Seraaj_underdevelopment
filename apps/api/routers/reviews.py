...
from models import (
    ...
    ReviewVote, ReviewFlag,
    User, Volunteer, Organisation, Application, Opportunity
)
...
async def create_review(
    ...
    existing_review = session.exec(
        select(Review).where(
            and_(
                ...
                or_(
                    and_(Review.volunteer_id == review_data.volunteer_id, review_data.volunteer_id.is_not(None)),
                    and_(Review.organisation_id == review_data.organisation_id, review_data.organisation_id.is_not(None))
                )
            )
        )
    ).first()
    ...
    await update_average_rating(
        session, 
        review_data.volunteer_id, 
        review_data.organisation_id
    )
...
async def get_review_statistics(
    ...
    recent_reviews = session.exec(
        ...
        .select_from(base_query.where(Review.created_at >= thirty_days_ago).subquery())
    ).first()
...
async def update_average_rating(
    ...
    if volunteer_id:
        ...
        avg_rating = session.exec(
            select(func.avg(Review.rating)).where(Review.organisation_id == organisation_id)
        ).first()
