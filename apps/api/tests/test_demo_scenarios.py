...
from models.demo_scenario import (
    DemoScenario, DemoStep, DemoRun, DemoTemplate, ScenarioType, 
    ScenarioStatus, ActionType, DemoUserType
)
...
class TestDemoScenariosAPI:
    ...
    def test_get_available_scenarios_with_filters(self, db_session: Session):
        ...
        response = client.get("/demo-scenarios/available?scenario_type=full_journey&difficulty_level=beginner")
        assert response.status_code == 200
        data = response.json()
        assert data["success"] is True
    ...
    def test_start_invalid_scenario(self):
        ...
        response = client.post(
            "/demo-scenarios/start/invalid-id",
            json={}
        )
...
class TestDemoScenarioService:
    ...
    def test_get_available_scenarios(self, db_session: Session):
        ...
        # Test with filters
        beginner_scenarios = service.get_available_scenarios(difficulty_level="beginner")
    ...
    def test_execute_demo_step(self, db_session: Session):
        ...
        # Execute step
        result = service.execute_demo_step(
    ...
    def test_create_scenario_from_template(self, db_session: Session):
        ...
        created_scenario = db_session.query(DemoScenario).filter(
            DemoScenario.scenario_id == scenario_id
        ).first()
...
class TestDemoScenariosIntegration:
    ...
    def test_full_demo_workflow(self, db_session: Session):
        ...
        # 4. Execute steps
        for step_num in [1, 2]:
            step_result = service.execute_demo_step(
                run_id=run_id,
