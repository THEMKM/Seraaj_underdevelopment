...
async def upload_file(
    ...
    if file.size and file.size > 10 * 1024 * 1024:
        raise_bad_request(
            ...
            details={
                "max_size": "10MB",
                "uploaded_size": f"{file.size / 1024 / 1024:.2f}MB",
            },
        )
...
async def download_file(
    ...
    try:
        ...
        return FileResponse(
            path=str(file_path), filename=db_file.filename, media_type=db_file.mime_type
        )
...
async def get_file_thumbnail(
    ...
    try:
        ...
        if not thumbnail_path or not thumbnail_path.exists():
            ...
            file_path = file_handler.get_file_path(db_file)
            if file_path.exists():
                return FileResponse(path=str(file_path), media_type=db_file.mime_type)
            else:
                raise HTTPException(
        ...
        return FileResponse(path=str(thumbnail_path), media_type=db_file.mime_type)
...
async def get_file_permissions(
    ...
    return {"file_id": file_id, "permissions": permission_list}
...
async def get_file_access_log(
    ...
    return {"file_id": file_id, "access_log": log_entries}
