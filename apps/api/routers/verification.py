...
from models import (
    ...
    SkillVerificationCreate, SkillVerificationRead, SkillVerificationUpdate
)
from routers.auth import get_current_user
from verification.skill_verifier import (
    verification_engine, VerificationMethod, VerificationStatus, BadgeType
)
from verification.trust_system import (
    trust_system, TrustLevel, VerificationBadge, TrustFactor
)
...
async def get_my_skill_verifications(
    ...
    query = select(SkillVerification).where(SkillVerification.volunteer_id == volunteer.id)
...
async def get_skill_verification_details(
    ...
    return {
        ...
        "assessment_results": verification.assessment_results if current_user.role == "admin" else None,
        "created_at": verification.created_at.isoformat(),
        "verified_at": verification.verified_at.isoformat() if verification.verified_at else None,
        "can_endorse": (
            current_user.role == "volunteer" and 
            verification.verification_method == VerificationMethod.PEER_ENDORSEMENT and
            verification.status == VerificationStatus.PENDING and
            verification.volunteer_id != volunteer.id if volunteer else False
        )
    }
...
async def complete_assessment_test(
    ...
    try:
        ...
        if success:
            ...
            return {"message": "Assessment completed but verification failed", "verified": False}
...
async def upload_verification_document(
    ...
    valid_document_types = [
        "certificate", "transcript", "portfolio", "work_sample", 
        "reference_letter", "project_documentation", "other"
    ]
...
async def get_verification_leaderboard(
    ...
    leaderboard = []
    for i, (user_id, first_name, last_name, badge_count, total_points) in enumerate(results):
        ...
        leaderboard.append({
            ...
            "top_badges": [{"name": name, "rarity": rarity} for name, rarity in top_badges]
        })
...
async def admin_verify_skill(
    ...
    # Update verification
    verification.status = VerificationStatus.VERIFIED if decision == "approve" else VerificationStatus.REJECTED
    ...
    if decision == "approve":
        ...
        await verification_engine._check_badge_eligibility(session, verification.volunteer_id)
    ...
    try:
        if recalculate or not organization.trust_score:
            ...
            trust_data = {
                ...
                "last_updated": organization.trust_last_updated.isoformat() if organization.trust_last_updated else None
            }
...
async def award_verification_badge(
    ...
    try:
        ...
        return {
            "message": "Verification badge awarded successfully",
            **badge_award
        }
    ...
    try:
        ...
        for org in organizations:
            ...
            org_list.append({
                "id": org.id,
                "name": org.name,
                "description": org.description[:200] + "..." if len(org.description or "") > 200 else org.description,
                ...
                "country": org.country,
                "city": org.city,
                "cause_areas": org.cause_areas[:3] if org.cause_areas else []  # Show top 3 causes
            })
        
        return {
            "top_organizations": org_list,
            "total_count": len(org_list)
        }
