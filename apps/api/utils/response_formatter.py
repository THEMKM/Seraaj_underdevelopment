...
class APIResponse:
    ...
    def loading(
        ...
        return JSONResponse(
            ...
            content=response_data,
            headers={
                "X-Operation-ID": loading_state.operation_id,
                "X-Loading": "true"
            }
        )
...
class MetricsCollector:
    ...
    def add_performance_metrics(
        ...
        response_data["performance"] = {
            ...
            "cache_hit_rate": round(cache_hits / max(database_query_count, 1) * 100, 2) if database_query_count > 0 else 0
        }
...
def created_resource(data: Any, resource_type: str, resource_id: str = None) -> JSONResponse:
...
def add_performance_headers(func):
    ...
    async def wrapper(*args, **kwargs):
        ...
        response.headers["X-Timestamp"] = datetime.now(datetime.timezone.utc).isoformat()
