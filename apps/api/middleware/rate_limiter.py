...
class TokenBucketStrategy(RateLimitStrategy):
    ...
    def __init__(self, limit: int, window_seconds: int, refill_rate: Optional[int] = None):
    ...
    async def is_allowed(self, key: str) -> tuple[bool, Dict[str, Any]]:
        ...
        metadata = {
            ...
            'retry_after': int(self.window_seconds - (bucket['tokens'] * self.window_seconds / self.refill_rate)) if not allowed else 0
        }
...
class SlidingWindowStrategy(RateLimitStrategy):
    ...
    async def is_allowed(self, key: str) -> tuple[bool, Dict[str, Any]]:
        ...
        reset_time = int(window[0] + self.window_seconds) if window else int(current_time + self.window_seconds)
...
class AdaptiveRateLimiter:
    ...
    async def check_rate_limit(
        ...
        if not allowed:
            await self._log_rate_limit_event(request, client_key, endpoint_type, metadata)
    ...
    async def _calculate_effective_limit(
        ...
        base_config = self.base_limits.get(endpoint_type, self.base_limits['api_general'])
        base_limit = base_config['limit']
        
        # Apply role multiplier
        role_multiplier = self.role_multipliers.get(user_role, 1.0) if user_role else 0.5
        ...
        effective_limit = int(base_limit * role_multiplier * premium_multiplier * self.system_load_factor)
    ...
    def _get_strategy(self, endpoint_type: str, effective_config: Dict[str, Any]) -> RateLimitStrategy:
    ...
    async def _detect_suspicious_activity(
        ...
        if requests_per_minute > 100:
            await self._handle_suspicious_activity(
                client_ip, 'high_frequency', {'requests_per_minute': requests_per_minute}
            )
        
        # Pattern 2: Hitting too many different endpoints
        if len(pattern['endpoints']) > 50:
            await self._handle_suspicious_activity(
                client_ip, 'endpoint_scanning', {'unique_endpoints': len(pattern['endpoints'])}
            )
    ...
    async def _handle_suspicious_activity(
        ...
        logger.warning(
            ...
            extra={'client_ip': client_ip, 'activity_type': activity_type, 'details': details}
        )
