...
class HealthChecker:
    ...
    def __init__(self):
        ...
        self.system_metrics: Dict[str, HealthMetric] = {
            "cpu_usage": HealthMetric(
                "CPU Usage", 0, "%", HealthStatus.HEALTHY, 70, 90
            ),
            "memory_usage": HealthMetric(
                "Memory Usage", 0, "%", HealthStatus.HEALTHY, 80, 95
            ),
            "disk_usage": HealthMetric(
                "Disk Usage", 0, "%", HealthStatus.HEALTHY, 85, 95
            ),
            "response_time": HealthMetric(
                "Avg Response Time", 0, "ms", HealthStatus.HEALTHY, 1000, 3000
            ),
            "error_rate": HealthMetric(
                "Error Rate", 0, "%", HealthStatus.HEALTHY, 1, 5
            ),
            "active_connections": HealthMetric(
                "Active Connections", 0, "count", HealthStatus.HEALTHY, 800, 1000
            ),
        }
    ...
    async def _check_email_health(self, service_health: ServiceHealth):
        ...
        try:
            ...
            try:
                ...
                with smtplib.SMTP(
                    settings.email.smtp_host, settings.email.smtp_port, timeout=10
                ) as server:
    ...
    async def _check_websocket_health(self, service_health: ServiceHealth):
        ...
        service_health.message = (
            f"WebSocket service healthy: {active_operations} active operations"
        )
    ...
    async def run_comprehensive_check(
        ...
        for i, service_type in enumerate(self.services.keys(), 1):
            ...
            try:
                await self._check_service_health(service_type)
                results["services"][service_type.value] = self.services[
                    service_type
                ].to_dict()
        ...
        results["summary"] = {
            ...
            "healthy_services": len(
                [s for s in self.services.values() if s.status == HealthStatus.HEALTHY]
            ),
    ...
    async def _collect_system_metrics(self):
        ...
        try:
            ...
            self.system_metrics["cpu_usage"].update(
                cpu_percent, f"CPU at {cpu_percent}%"
            )
            ...
            self.system_metrics["memory_usage"].update(
                memory.percent,
                f"Memory at {memory.percent}% ({memory.used / 1024**3:.1f}GB used)",
            )
            ...
            self.system_metrics["disk_usage"].update(
                disk_percent,
                f"Disk at {disk_percent}% ({disk.free / 1024**3:.1f}GB free)",
            )
