...
class OrganizationTrustSystem:
    ...
    async def calculate_trust_score(
        ...
        trust_factors[TrustFactor.REGISTRATION_STATUS] = (
            self._evaluate_registration_status(session, organization)
        )

        # 2. Financial Transparency (15%)
        trust_factors[TrustFactor.FINANCIAL_TRANSPARENCY] = (
            self._evaluate_financial_transparency(session, organization)
        )

        # 3. Volunteer Feedback (15%)
        trust_factors[TrustFactor.VOLUNTEER_FEEDBACK] = (
            await self._evaluate_volunteer_feedback(session, organization_id)
        )
        ...
        trust_factors[TrustFactor.LEADERSHIP_CREDIBILITY] = (
            self._evaluate_leadership_credibility(session, organization)
        )

        # 6. Operational History (8%)
        trust_factors[TrustFactor.OPERATIONAL_HISTORY] = (
            self._evaluate_operational_history(session, organization)
        )

        # 7. External Endorsements (8%)
        trust_factors[TrustFactor.EXTERNAL_ENDORSEMENTS] = (
            self._evaluate_external_endorsements(session, organization)
        )

        # 8. Platform Engagement (7%)
        trust_factors[TrustFactor.PLATFORM_ENGAGEMENT] = (
            await self._evaluate_platform_engagement(session, organization_id)
        )
        ...
        trust_factors[TrustFactor.INNOVATION_LEADERSHIP] = (
            self._evaluate_innovation_leadership(session, organization)
        )
        ...
        return {
            ...
            "trust_factors": {
                factor.value: round(score, 3) for factor, score in trust_factors.items()
            },
            "badge_bonus": round(badge_bonus, 3),
            "calculated_at": datetime.now(datetime.timezone.utc).isoformat(),
        }

    def _evaluate_registration_status(
        self, session: Session, organization: Organisation
    ) -> float:
        ...
        if organization.created_at:
            age_months = (
                datetime.now(datetime.timezone.utc) - organization.created_at
            ).days / 30
            if age_months >= 12:
                score += 0.3
            elif age_months >= 6:
    ...
    def _evaluate_financial_transparency(
        self, session: Session, organization: Organisation
    ) -> float:
    ...
    async def _evaluate_volunteer_feedback(
        self, session: Session, organization_id: int
    ) -> float:
        ...
        # Review count bonus
        review_count_bonus = min(
            len(reviews) / 50, 0.2
        )  # Up to 20% bonus for 50+ reviews
    ...
    def _evaluate_community_impact(
        self, session: Session, organization: Organisation
    ) -> float:
    ...
    def _evaluate_leadership_credibility(
        self, session: Session, organization: Organisation
    ) -> float:
    ...
    def _evaluate_operational_history(
        self, session: Session, organization: Organisation
    ) -> float:
        ...
        # Platform tenure
        age_months = (
            (datetime.now(datetime.timezone.utc) - organization.created_at).days / 30
            if organization.created_at
            else 0
        )
        if age_months >= 24:
    ...
    def _evaluate_external_endorsements(
        self, session: Session, organization: Organisation
    ) -> float:
    ...
    async def _evaluate_platform_engagement(
        self, session: Session, organization_id: int
    ) -> float:
        ...
        if applications:
            responded_count = len(
                [a for a in applications if a.status not in ["submitted", "draft"]]
            )
    ...
    def _evaluate_compliance_record(
        self, session: Session, organization: Organisation
    ) -> float:
    ...
    def _evaluate_innovation_leadership(
        self, session: Session, organization: Organisation
    ) -> float:
    ...
    async def _calculate_badge_bonus(
        self, session: Session, organization_id: int
    ) -> float:
    ...
    async def check_badge_eligibility(
        ...
        current_badges = session.exec(
            select(UserBadge.badge_code).where(
        ...
        for badge_code, badge_info in self.badge_criteria.items():
            ...
            if await self._meets_badge_criteria(
                session, organization, badge_info["criteria"]
            ):
    ...
    async def _meets_badge_criteria(
        self, session: Session, organization: Organisation, criteria: Dict[str, Any]
    ) -> bool:
            ...
            if criterion == "registration_documents":
                ...
                review_count = session.exec(
                    select(func.count(Review.id)).where(
                        Review.organisation_id == organization.id
                    )
                ).first()
    ...
    async def award_verification_badge(
        ...
        logger.info(
            f"Verification badge '{badge_code}' awarded to organization {organization_id}"
        )
        ...
        try:
            event = AnalyticsEvent(
                ...
                data={
                    "organization_id": organization_id,
                    "trust_factors": {
                        factor.value: score for factor, score in trust_factors.items()
                    },
    ...
    def get_trust_system_stats(self, session: Session) -> Dict[str, Any]:
        ...
        trust_distribution = session.exec(
            select(Organisation.trust_level, func.count(Organisation.id)).group_by(
                Organisation.trust_level
            )
        ).all()
        ...
        return {
            ...
            "trust_weights": {
                factor.value: weight for factor, weight in self.trust_weights.items()
            },
            "trust_thresholds": {
