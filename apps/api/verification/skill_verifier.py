...
from models import (
    User, Volunteer, SkillVerification, Badge, UserBadge,
    AnalyticsEvent, Review, Application, Opportunity
)
...
class SkillVerificationEngine:
        ...
        return {
            "skill_master": {
                ...
                "criteria": {
                    ...
                    "required_methods": [VerificationMethod.ASSESSMENT_TEST, VerificationMethod.PORTFOLIO_REVIEW],
                    "peer_endorsements": 3
                },
                "rarity": "rare",
                "points": 100
    ...
    async def initiate_skill_verification(
        ...
        existing_verification = session.exec(
            select(SkillVerification).where(
                and_(
                    ...
                    SkillVerification.status.in_([VerificationStatus.PENDING, VerificationStatus.IN_REVIEW])
                )
            )
        ).first()
        
        if existing_verification:
            raise ValueError(f"Active verification already exists for skill: {skill_name}")
        ...
        await self._log_verification_event(
            session, verification.id, "verification_initiated", 
            {"method": verification_method.value, "skill": skill_name}
        )
        
        # Auto-process certain verification types
        if verification_method in [VerificationMethod.CERTIFICATE_UPLOAD, VerificationMethod.WORK_SAMPLE]:
            await self._auto_process_verification(session, verification)
        
        logger.info(f"Skill verification initiated: {skill_name} for volunteer {volunteer_id}")
        return verification
    ...
    async def _auto_process_verification(
        ...
        verification.status = VerificationStatus.VERIFIED if score >= 0.7 else VerificationStatus.REJECTED
    ...
    async def _check_endorsement_completion(
        ...
        for endorsement in endorsements:
            ...
            weight = {"basic": 0.5, "intermediate": 0.7, "advanced": 0.9, "expert": 1.0}.get(endorser_level, 0.5)
        ...
        verification.status = VerificationStatus.VERIFIED if final_score >= 0.7 else VerificationStatus.REJECTED
        verification.verified_at = datetime.now(datetime.timezone.utc)
    ...
    async def complete_assessment_test(
        ...
        verification.status = VerificationStatus.VERIFIED if score >= 0.6 else VerificationStatus.REJECTED
        verification.verified_at = datetime.now(datetime.timezone.utc)
    ...
    async def _update_volunteer_skills(
        ...
        for i, skill in enumerate(skills):
            if isinstance(skill, dict) and skill.get("name") == verification.skill_name:
                ...
                current_level_value = {"basic": 1, "intermediate": 2, "advanced": 3, "expert": 4}.get(skill.get("level", "basic"), 1)
                new_level_value = {"basic": 1, "intermediate": 2, "advanced": 3, "expert": 4}.get(skill_level, 1)
                ...
                if new_level_value > current_level_value:
                    skills[i] = {
                        ...
                        "level": skill_level,
                        "verified": True,
                        "verification_date": datetime.now(datetime.timezone.utc).isoformat(),
    ...
    async def _meets_badge_criteria(
            ...
            verifications = session.exec(
                select(SkillVerification).where(
                    and_(
                        ...
                        SkillVerification.status == VerificationStatus.VERIFIED,
                        SkillVerification.verification_score >= badge_criteria["min_verification_score"]
                    )
                )
            ).all()
        ...
        if "required_methods" in badge_criteria:
            ...
            volunteer_methods = session.exec(
                select(SkillVerification.verification_method).where(
                    and_(
                        SkillVerification.volunteer_id == volunteer.id,
                        SkillVerification.status == VerificationStatus.VERIFIED
                    )
        ...
        if "peer_endorsements_given" in badge_criteria:
            endorsements_given = session.exec(
                select(func.count(SkillVerification.id)).where(
                    SkillVerification.peer_endorsements.op("@>")([{"endorser_id": volunteer.user_id}])
                )
            ).first()
    ...
    async def get_verification_statistics(
        ...
        if volunteer_id:
            base_query = base_query.where(SkillVerification.volunteer_id == volunteer_id)
        ...
        status_stats = session.exec(
            select(SkillVerification.status, func.count(SkillVerification.id))
            .group_by(SkillVerification.status)
        ).all()
        
        # Verifications by method
        method_stats = session.exec(
            select(SkillVerification.verification_method, func.count(SkillVerification.id))
            .group_by(SkillVerification.verification_method)
        ).all()
        
        # Average verification scores
        avg_scores = session.exec(
            select(SkillVerification.verification_method, func.avg(SkillVerification.verification_score))
        ...
        return {
            ...
            "average_scores_by_method": {method.value: round(float(score), 3) for method, score in avg_scores},
