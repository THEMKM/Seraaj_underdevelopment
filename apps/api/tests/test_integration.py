...
from models import User, Opportunity, Application, Review
...
class TestOrganizationJourney:
    ...
    def test_complete_organization_workflow(
        ...
        publish_response = client.post(
            f"/v1/opportunities/{opportunity_id}/publish",
            headers=org_headers
        )
        ...
        analytics_response = client.get(
            "/v1/opportunities/analytics/performance",
            headers=org_headers
        )
...
class TestReviewAndRatingSystem:
    ...
    def test_complete_review_workflow(
        ...
        application = Application(
            volunteer_id=session.query(Volunteer).filter(Volunteer.user_id == test_user_volunteer.id).first().id,
        ...
        review_data = {
            "organisation_id": test_opportunity.org_id,
            **sample_review_data
        }
...
class TestMatchingSystem:
    ...
    def test_opportunity_recommendations(
        ...
        recommendations = client.get(
            "/v1/opportunities/recommendations",
            headers=auth_headers_volunteer
        )
...
class TestFileUploadIntegration:
    ...
    def test_profile_picture_upload_workflow(
        ...
        file_data = {
            "file": ("profile.jpg", b"fake image content", "image/jpeg")
        }
        form_data = {
        ...
        if upload_response.status_code == 201:
            ...
            assert profile_response.status_code in [200, 400]  # May fail due to validation
    ...
    def test_bulk_operations_performance(
        ...
        for i in range(50):  # Create 50 opportunities
            ...
            response = client.post(
                "/v1/opportunities",
                json=opp_data,
                headers={"Authorization": f"Bearer mock_token_org_{test_user_organization.id}"}
            )
