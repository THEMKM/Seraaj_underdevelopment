...
class LoadingState:
    ...
    def update_progress(self, step: int = None, description: str = None, step_data: Dict[str, Any] = None):
        ...
        if self.current_step > 0 and self.total_steps:
            ...
            self.estimated_completion = time.time() + (remaining_steps * avg_time_per_step)
...
class LoadingStateManager:
    ...
    def create_loading_state(
        ...
        loading_state = LoadingState(operation_id, operation_type, total_steps, description)
    ...
    def complete_loading_state(self, operation_id: str, result: Dict[str, Any] = None) -> bool:
...
class ProgressTracker:
    ...
    def update(self, step: int = None, description: str = None, step_data: Dict[str, Any] = None):
        """Update progress"""
        loading_manager.update_loading_state(self.operation_id, step, description, step_data)
    
    def set_total_steps(self, total_steps: int):
        """Update total steps"""
        self.loading_state.total_steps = total_steps
        if self.loading_state.current_step > 0:
            self.loading_state.progress_percentage = (self.loading_state.current_step / total_steps) * 100
...
def track_progress(
    ...
    def decorator(func: Callable) -> Callable:
        async def wrapper(*args, **kwargs):
            ...
            with ProgressTracker(operation_id, operation_type, total_steps, description) as tracker:
