...
from models.push_notification import NotificationType, NotificationPriority
...
async def send_opportunity_match_notification(
    user_id: int, opportunity_id: int, match_score: float
):
    ...
    try:
        with next(get_session()) as session:
            ...
            if not opportunity:
                logger.error(
                    f"Opportunity {opportunity_id} not found for match notification"
                )
                return
...
async def send_application_update_notification(
    ...
    try:
        with next(get_session()) as session:
            ...
            if not application:
                logger.error(
                    f"Application {application_id} not found for status notification"
                )
                return
            ...
            priority = (
                ...
                if status in ["accepted", "interview_scheduled"]
...
async def send_message_notification(
    user_id: int, sender_name: str, message_preview: str, conversation_id: int
):
...
async def send_deadline_reminder_notification(
    user_id: int, opportunity_title: str, deadline: datetime, opportunity_id: int
):
...
async def send_donation_received_notification(
    user_id: int, amount: float, currency: str, donor_name: str
):
    ...
    try:
        with next(get_session()) as session:
            ...
            await service.send_notification(
                ...
                data={"amount": amount, "currency": currency, "donor_name": donor_name},
                priority=NotificationPriority.NORMAL,
...
async def send_skill_verification_notification(
    user_id: int, skill_name: str, status: str
):
...
async def schedule_opportunity_deadline_reminders():
    ...
    try:
        with next(get_session()) as session:
            ...
            logger.info(
                f"Scheduled deadline reminders for {len(opportunities)} opportunities"
            )
