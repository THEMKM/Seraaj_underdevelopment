...
from models import (
    ...
    User, Volunteer, Organisation, Opportunity, ApplicationStatus, FileUpload
)
...
async def create_application(
    ...
    if opportunity.state != "active":
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Opportunity is not active"
        )
...
async def upload_application_document(
    ...
    valid_document_types = [
        ...
        "reference_letter", "identification", "other"
    ]
...
async def complete_application_step(
    ...
    # Validate step name
    valid_steps = [
        "basic_info", "motivation", "availability", "experience",
        "references", "background_check", "skills_assessment"
    ]
    ...
    completed_steps = [step for step, info in application_steps.items() 
...
async def get_application_steps(
    ...
    for step_name, step_info in all_steps.items():
        ...
        steps_status.append({
            ...
            "data": step_status.get("data") if current_user.role != "organization" else None  # Hide data from orgs
        })
    ...
    progress = {
        ...
        "required_progress_percent": round((len(completed_required) / len(required_steps)) * 100, 1) if required_steps else 100
    }
...
async def get_applications_stats(
    ...
    if current_user.role == "volunteer":
        ...
        if volunteer:
            ...
            total = session.exec(
                select(func.count(Application.id)).where(Application.volunteer_id == volunteer.id)
            ).first()
