...
from models.demo_scenario import (
    DemoScenario, DemoStep, DemoRun, DemoTemplate, DemoAnalytics,
    DemoFeedback, ScenarioType, ScenarioStatus, ActionType, DemoUserType
)
...
class DemoScenarioService:
    ...
    async def get_available_scenarios(
        ...
        try:
            query = select(DemoScenario).where(DemoScenario.status == ScenarioStatus.ACTIVE)
            ...
            scenarios = self.session.exec(query.order_by(DemoScenario.total_runs.desc())).all()
    ...
    async def start_demo_scenario(
        ...
        try:
            ...
            demo_run = DemoRun(
                ...
                device_type=environment_info.get("device_type") if environment_info else None,
                screen_resolution=environment_info.get("screen_resolution") if environment_info else None,
                user_agent=environment_info.get("user_agent") if environment_info else None
            )
    ...
    async def execute_demo_step(
        ...
        try:
            ...
            if not is_complete and step_result["success"]:
                next_step = await self._get_scenario_step(demo_run.scenario_id, step_number + 1)
            ...
            return {
                ...
                "progress": {
                    ...
                    "completion_percentage": (len(demo_run.completed_steps) / total_steps) * 100
                }
            }
        ...
        try:
            ...
            if demo_run.started_at:
                demo_run.total_duration_seconds = int(
                    (datetime.now(datetime.timezone.utc) - demo_run.started_at).total_seconds()
                )
    ...
    async def get_scenario_analytics(
        ...
        try:
            ...
            avg_duration = None
            if completed_runs:
                avg_duration = sum(r.total_duration_seconds for r in completed_runs) / len(completed_runs)
            ...
            for run in runs:
                ...
                if run.device_type:
                    device_stats[run.device_type] = device_stats.get(run.device_type, 0) + 1
            ...
            for step in scenario.steps:
                step_data = {
                    ...
                    "success_rate": (step.success_count / (step.success_count + step.failure_count) * 100) if (step.success_count + step.failure_count) > 0 else 0,
            ...
            return {
                ...
                "recent_feedback": await self._get_recent_feedback(scenario.id, limit=5)
            }
    ...
    async def create_scenario_from_template(
        ...
        try:
            ...
            scenario = DemoScenario(
                ...
                scenario_type=ScenarioType(customizations.get("scenario_type", template_data["scenario_type"])),
                target_audience=customizations.get("target_audience", template_data.get("target_audience", "general")),
                duration_minutes=customizations.get("duration_minutes", template_data.get("duration_minutes", 10)),
                difficulty_level=customizations.get("difficulty_level", template_data.get("difficulty_level", "beginner")),
                auto_play=customizations.get("auto_play", template_data.get("auto_play", False)),
            ...
            for step_data in template_data.get("steps", []):
                step = DemoStep(
                    ...
                    demo_user_type=DemoUserType(step_data["demo_user_type"]) if step_data.get("demo_user_type") else None,
            ...
            logger.info(f"Created scenario {scenario.scenario_id} from template {template_id}")
    ...
    async def _get_scenario_step(self, scenario_id: int, step_number: int) -> Optional[Dict[str, Any]]:
    ...
    async def _execute_step_action(
        ...
        try:
            ...
            result = {"success": True, "message": f"Executed {action_type} successfully"}
            ...
            execution_time = (datetime.now(datetime.timezone.utc) - step_start_time).total_seconds()
            result["execution_time"] = execution_time
            
            # Update step analytics
            await self._update_step_analytics(step["id"], result["success"], execution_time)
    ...
    async def _simulate_create_user(self, step: Dict[str, Any], context: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    async def _simulate_login(self, step: Dict[str, Any], context: Optional[Dict[str, Any]]) -> Dict[str, Any]:
        """Simulate user login"""
        demo_user_type = step.get("demo_user_type")
        session_id = context.get("session_id") if context else "default"
        
        # Get demo user from cache
        if session_id in self.demo_users and demo_user_type in self.demo_users[session_id]:
    ...
    async def _simulate_navigate(self, step: Dict[str, Any], context: Optional[Dict[str, Any]]) -> Dict[str, Any]:
        """Simulate navigation"""
    ...
    async def _simulate_fill_form(self, step: Dict[str, Any], context: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    async def _simulate_click_button(self, step: Dict[str, Any], context: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    async def _simulate_upload_file(self, step: Dict[str, Any], context: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    async def _simulate_send_message(self, step: Dict[str, Any], context: Optional[Dict[str, Any]]) -> Dict[str, Any]:
        """Simulate sending a message"""
        return {
            "success": True,
            "message": "Message sent successfully",
            "data": {"message_id": f"msg_{datetime.now(datetime.timezone.utc).timestamp()}"}
        }
    
    async def _simulate_make_payment(self, step: Dict[str, Any], context: Optional[Dict[str, Any]]) -> Dict[str, Any]:
        """Simulate payment processing"""
        return {
            "success": True,
            "message": "Payment processed successfully",
            "data": {"transaction_id": f"txn_{datetime.now(datetime.timezone.utc).timestamp()}"}
        }
    
    async def _simulate_submit_application(self, step: Dict[str, Any], context: Optional[Dict[str, Any]]) -> Dict[str, Any]:
        """Simulate application submission"""
        return {
            "success": True,
            "message": "Application submitted successfully",
            "data": {"application_id": f"app_{datetime.now(datetime.timezone.utc).timestamp()}"}
        }
    
    async def _simulate_create_opportunity(self, step: Dict[str, Any], context: Optional[Dict[str, Any]]) -> Dict[str, Any]:
        """Simulate opportunity creation"""
        return {
            "success": True,
            "message": "Opportunity created successfully",
            "data": {"opportunity_id": f"opp_{datetime.now(datetime.timezone.utc).timestamp()}"}
        }
    ...
    async def _complete_demo_run(self, demo_run: DemoRun):
        ...
        if scenario:
            ...
            successful_runs = len([r for r in all_completed_runs if r.status == "completed"])
            scenario.success_rate = (successful_runs / len(all_completed_runs)) * 100 if all_completed_runs else 0
            
            self.session.add(scenario)
        
        self.session.commit()
    
    async def _update_step_analytics(self, step_id: int, success: bool, execution_time: float):
        """Update step analytics"""
            ...
            if step:
                ...
                if step.average_execution_time is None:
                    ...
                    step.average_execution_time = (
                        (step.average_execution_time * (total_executions - 1) + execution_time) / total_executions
                    )
    ...
    async def _get_recent_feedback(self, scenario_id: int, limit: int = 5) -> List[Dict[str, Any]]:
        """Get recent feedback for a scenario"""
        
        feedback_records = self.session.exec(
            select(DemoFeedback).where(DemoFeedback.scenario_id == scenario_id)
...
def get_demo_scenario_service(session: Session = Depends(get_session)) -> DemoScenarioService:
