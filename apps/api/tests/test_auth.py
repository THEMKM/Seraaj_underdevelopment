...
class TestAuthenticationEndpoints:
    ...
    def test_user_registration_volunteer_success(self, client: TestClient, session: Session):
        ...
        user = session.query(User).filter(User.email == registration_data["email"]).first()
        assert user is not None
        assert user.role == "volunteer"
        assert not user.verified  # Should require email verification
    
    def test_user_registration_organization_success(self, client: TestClient, session: Session):
    ...
    def test_user_registration_duplicate_email(self, client: TestClient, test_user_volunteer: User):
    ...
    def test_user_login_invalid_credentials(self, client: TestClient, test_user_volunteer: User):
    ...
    def test_get_current_user_authenticated(self, client: TestClient, auth_headers_volunteer: dict):
    ...
    def test_refresh_token_success(self, client: TestClient, auth_headers_volunteer: dict):
    ...
    def test_change_password_success(self, client: TestClient, auth_headers_volunteer: dict):
    ...
    def test_change_password_wrong_current(self, client: TestClient, auth_headers_volunteer: dict):
    ...
    def test_password_reset_request(self, client: TestClient, test_user_volunteer: User):
...
class TestAuthorizationMiddleware:
    ...
    def test_admin_only_endpoint_as_admin(self, client: TestClient, auth_headers_admin: dict):
    ...
    def test_admin_only_endpoint_as_volunteer(self, client: TestClient, auth_headers_volunteer: dict):
        """Test admin-only endpoint access as volunteer"""
        response = client.get("/v1/admin/users", headers=auth_headers_volunteer)
        assert response.status_code == 403
    
    def test_organization_only_endpoint_as_organization(self, client: TestClient, auth_headers_organization: dict):
    ...
    def test_organization_only_endpoint_as_volunteer(self, client: TestClient, auth_headers_volunteer: dict):
...
class TestSecurityFeatures:
    ...
    def test_rate_limiting_login_attempts(self, client: TestClient, test_user_volunteer: User):
